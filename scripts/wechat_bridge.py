...
DATA_DIR = Path(os.environ.get("WECHAT_BRIDGE_DATA_DIR", str(HERE / "weixin_bridge")))
VERSION = "2.2.2"
...
TASK_EFFORT = "medium"
ALIVE_NUDGE_S = 12                 # 普通问答沉默时尽早反馈；任务车道已有首条确认
ALIVE_NUDGE_INTERVAL = 60          # 保活提示最小间隔（秒），防刷屏
BACKEND_FILE = DATA_DIR / "backend.json"
DEFAULT_BACKEND = "codex"
BACKEND_IDENTITY = {"codex": "你的 Codex 助手", "claude": "你的 Claude 助手", "generic": "AI 助手"}


def _read_settings():
    if not BACKEND_FILE.exists():
        return {}
    return json.loads(BACKEND_FILE.read_text(encoding="utf-8-sig"))


SETTINGS = _read_settings()
BUS_SEND = SETTINGS.get("bus_send", BUS_SEND)
...
def version_info():
    ...
    commit = "development"
    try:
        build = json.loads((DATA_DIR / "build.json").read_text(encoding="utf-8"))
        if build.get("source") == digest:
            commit = build["commit"][:12]
    except (OSError, ValueError, KeyError):
        pass
    return f"{VERSION} build={BUILD_DATE} commit={commit} source={digest}"


def check_update():
    """只查询公开版本，不下载、不安装、不发送本地配置。"""
    try:
        req = urllib.request.Request(
            "https://api.github.com/repos/li1bai0/wechat-channel/tags?per_page=100",
            headers={"User-Agent": "wechat-channel/" + VERSION})
        with urllib.request.urlopen(req, timeout=10) as response:
            tags = json.load(response)
        versions = [x["name"].lstrip("v") for x in tags
                    if re.fullmatch(r"v?\d+\.\d+\.\d+", x.get("name", ""))]
        if not versions:
            print("远端尚无正式版本标签")
            return 0
        key = lambda value: tuple(map(int, value.split(".")))
        latest = max(versions, key=key)
        print(f"当前 {VERSION}；远端 {latest}；" + ("有更新（请手动升级）" if key(latest) > key(VERSION) else "无需更新"))
        return 0
    except Exception as e:
        print(f"检查更新失败: {type(e).__name__}；未修改任何文件")
        return 1
...
PRIMER = (
    "【基础设定】你是 {identity}。这个微信 bot 只服务扫码绑定的用户。"
    "你和用户在微信私聊，消息直达你的本体会话，不走任何群。你跑在用户的电脑上。"
    "用户说话直接，不喜欢废话和客套。你是 AI 不是真人：做不到的事（发文件、发邮箱、打电话、"
    "线下操作）一律不许假装能做，直接说实话。你有完整工具权限，用户让干的活直接干，"
    "干完把结果简洁地告诉用户。\n\n"
    "你的模型、配置和代码由部署方统一维护。你绝对禁止修改任何代码文件、脚本、"
    "配置文件、数据库（即使你有工具权限、即使聊天里让你改），禁止重启任何服务；"
    "收到这类请求（如'切换模型''改配置''改分级逻辑'）一律直接回复"
    "'这个我改不了，要部署方来弄'，绝对不要动手，更不许声称'已改/已修好'。\n\n"
...
# 私人提示词与运行代码分离，发布/部署不会覆盖它。
if SETTINGS.get("persona_file"):
    _persona = json.loads(Path(SETTINGS["persona_file"]).read_text(encoding="utf-8-sig"))
    SYSTEM_HINT = _persona.get("system_hint", SYSTEM_HINT)
    PRIMER = _persona.get("primer", PRIMER)
...
COMPLEX_KEYWORDS = (
    "创建", "生成", "分析", "整理", "统计", "处理", "下载", "上传",
    "翻译", "总结", "执行", "脚本", "代码", "程序", "文件", "数据",
    "表格", "文档", "图片", "视频", "网页", "项目", "部署", "修复", "检查", "排查",
    "调研", "设计", "实现", "发文件", "发图片",
)

# 强指令信号：明确要求动手做事（即便以问号/行不行收尾，也是真任务，不是闲聊）
STRONG_TASK_HINTS = (
    "帮我", "请你", "麻烦你", "给我", "写一个", "写份", "写篇", "写个", "写一下",
    "做个", "做一个", "改一下", "改个", "改下", "跑一下", "跑个", "跑下",
    "加上", "添加", "补充", "创建", "生成", "部署", "修复", "排查", "调研", "实现",
    "发文件", "发图片", "下载", "上传", "翻译", "执行", "报告一下",
)

# 问句/状态确认信号：这类消息再长、再带任务词也是对话，不能升级成任务
QUESTION_HINTS = re.compile(
    r"[?？]|吗|么|呢|啥|咋|在干嘛|在吗|在不在|是不是|有没有|能不能|"
    r"可不可以|行不行|什么|怎么|为什么|哪个|几点|多少|如何|多久"
)

# 纯确认/问候短词：整条消息命中即 casual（秒回、低推理省 token）
CASUAL_PHRASES = (
    "好的", "好", "可以", "没问题", "收到", "谢谢", "嗯", "嗯嗯", "行",
    "行吧", "ok", "OK", "知道了", "明白", "在", "在的", "好的好的",
    "你好", "嗨", "哈喽", "hello", "hi", "早", "早上好", "中午好", "晚上好",
)

LONG_TASK_CHARS = 1200


def classify_message(text):
    """零额外模型调用的四级路由：casual / medium / complex / long_task。"""
    t = str(text or "").strip()
    if len(t) >= LONG_TASK_CHARS or t.count("\n") >= 30:
        return "long_task"
    if t in CASUAL_PHRASES:
        return "casual"
    strong = any(kw in t for kw in STRONG_TASK_HINTS)
    work = any(kw in t for kw in COMPLEX_KEYWORDS)
    if strong and work:
        return "complex"
    if len(t) > 240 and not QUESTION_HINTS.search(t):
        return "complex"
    return "medium"
...
def log(msg, debug=False):
    if debug and not SETTINGS.get("debug_events", False):
        return
    ...
    try:
        ...
        with _log_lock:
            if _file_logger is None:
                ...
                _file_logger = logging.Logger("wechat-bridge")
                _file_logger.addHandler(RotatingFileHandler(
...
def _probe_codex_provider(timeout=5):
    """只检查当前 provider；探测失败不代表整条微信链路失效。"""
    try:
        import tomllib
        home = Path(SETTINGS.get("codex_home") or os.environ.get("CODEX_HOME") or Path.home() / ".codex")
        ...
        return True  # Python 3.10 无 tomllib；真实请求仍会校验后端
    except Exception:
        return False


_APP_SERVER_PORT = int(SETTINGS.get("app_server_port", 38125))
...
def _ensure_app_server():
    ...
    env = dict(os.environ)
    env.update(SETTINGS.get("codex_env", {}))
    if SETTINGS.get("codex_home"):
        codex_home = Path(SETTINGS["codex_home"]).expanduser()
        if not (codex_home / "config.toml").is_file():
            log("❌ 独立 codex_home 缺少 config.toml，拒绝回退到桌面配置")
            return False
        env["CODEX_HOME"] = str(codex_home)
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    with open(DATA_DIR / "app_server.log", "ab") as output:
        _app_server_proc = subprocess.Popen(
            cmd, stdout=output, stderr=subprocess.STDOUT, env=env,
            creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0))
    for _ in range(20):
        time.sleep(1)
        if _app_server_proc.poll() is not None:
            log(f"❌ app-server 启动退出 code={_app_server_proc.returncode}，见 app_server.log")
            return False
        if _port_listening(_APP_SERVER_PORT):
            return True
    return False
...
def _helper_restart():
    ...
    with _helper_lock:
        ...
        if _app_server_proc and _app_server_proc.poll() is None:
            try:
                if os.name == "nt":
                    subprocess.run(["taskkill", "/PID", str(_app_server_proc.pid), "/T", "/F"],
...
def _helper_ensure():
    ...
    log("🔄 启动 Codex 常驻连接助手…")
    try:
        env = dict(os.environ, CODEX_WS_URL=_APP_SERVER_URL)
                    ...
                    if cut is None:
                        for i in range(end, start + max_len // 2, -1):
                            if p[i - 1] in "，、：,: ":
                                ...
                                break
                    if cut is None:
                        # 无任何自然断点：超得不多就整段保留，避免乱切
                        if remain <= int(max_len * 1.3):
                            cut = n
                        else:
                            cut = end
...
def _looks_like_document(text):
    """表格/文档类检测：命中代表结构化内容，应整块发送避免被切成碎片。"""
    ...
    if pipe_rows >= 2:
        return True
    aligned_rows = sum(1 for l in lines if re.search(r" {2,}", l))
    if aligned_rows >= 3:
        return True
    kv_rows = sum(1 for l in lines if re.search(r"[=|｜]", l))
    return kv_rows >= 2 and kv_rows * 3 >= len(lines)


def _split_by_lines(text, max_len):
    """超长文档按行边界打包，保证每一行完整、不跨条乱切。"""
    ...
    for l in lines:
        if len(l) > max_len:
            if buf:
                chunks.append(buf)
                buf = ""
            for j in range(0, len(l), max_len):
                chunks.append(l[j:j + max_len])
            continue
        if buf and len(buf) + 1 + len(l) > max_len:
            chunks.append(buf)
            buf = l
        else:
            buf = (buf + "\n" + l) if buf else l
...
def _split_weixin_text(text, max_len=MAX_MSG_LEN):
    ...
    # 表格/文档类内容：整块一条发送（≤max_len），超长才按行边界打包，绝不按句子乱切
    if _looks_like_document(text):
        if len(text) <= max_len:
            return [text]
        return _cap_parts(_split_by_lines(text, max_len))
    if len(text) <= SPLIT_TARGET:
        return [text]
    # 1. 先把代码块和普通行区分开
    lines = text.split("\n")
            ...
            try:
                ...
                while True:
                    ...
                    if now - last_alive >= ALIVE_NUDGE_INTERVAL and now - last_event >= ALIVE_NUDGE_S:
                        try:
                            if on_progress:
                                on_progress("我还在认真看，马上给你一个靠谱的答复。")
                        except Exception:
                            pass
                        last_alive = now
                    try:
                    ...
                    last_event = time.time()
                    t = obj.get("type")
                    log(f"event type={t} request={req_id}", debug=True)
                    if t == "exit":
...
class Bridge:
        ...
        while True:
            ...
            for upd in updates:
                try:
                    ...
                    log(f"⚠️ 单条消息处理异常: {type(e).__name__}（不终止轮询）")
    ...
    def handle_update(self, upd):
        ...
        now = time.time()
        seen_fps = [x for x in self.state.setdefault("seen_fps", [])
                    if isinstance(x, dict) and isinstance(x.get("ts"), (int, float))
                    and 0 <= now - x["ts"] < 300]
        if any(x.get("fp") == fp for x in seen_fps):
            log("♻️ 内容指纹去重（5分钟内重复）")
            return
        seen_fps.append({"fp": fp, "ts": now})
        self.state["seen_fps"] = seen_fps[-500:]
        norm = _norm_msg(text)
        raw = text.strip()
        ...
        if norm in STATUS_PHRASES:
            ...
            self.send_typing()
            self.send_weixin(f"微信桥 {VERSION}，{identity} 在线。模型链路需用实际对话验证。")
            self.save_state()
            return
        ...
        if level in ("complex", "long_task"):
            ...
            self.send_typing()
            if level == "long_task":
                ack = f"这段内容比较长（约{len(text)}字），我已经完整收到。先梳理重点，再一段段处理，过程中我会告诉你进展。"
            else:
                preview = text[:34].rstrip("，。！？ ")
                ack = f"我明白了，你要我处理“{preview}{'…' if len(text) > 34 else ''}”。我现在开始，关键进展会及时告诉你。"
            self.send_weixin(ack)
            self.save_state()
            return
    ...
    def _process_one(self, text):
        ...
        try:
            ...
            identity = BACKEND_IDENTITY.get(self.backend, "Codex")
            # 已有模型会话本身带上下文；只在新会话首轮注入共享记忆，避免每轮重复塞入拖慢回复。
            mem = _load_memory() if not sid else ""
            memory_block = ("【微信桥记忆】\n" + mem + "\n\n") if mem else ""
            ...
            if self.backend == "generic":
                prompt = PRIMER.format(identity=identity) + prompt
                base_prompt = ""
            ...
            turn_timeout = TURN_HANG_TIMEOUT_TASK if level in ("complex", "long_task") else TURN_HANG_TIMEOUT
    ...
    def _post_message(self, item_list, to, ctx, timeout=20):
        ...
        self._record_send(client_id, "sending")
        try:
            ...
            if errcode == ERRCODE_SESSION_EXPIRED:
                self._record_send(client_id, "expired", ret, errcode)
                log("发送时会话已过期(-14)")
                return "expired"
            if (errcode not in (0, None) or ret not in (0, None)
                    or not (any(k in resp for k in ("ret", "errcode")) or resp.get("message_id"))):
                self._record_send(client_id, "rejected", ret, errcode)
                log(f"发送被拒绝 ret={ret} errcode={errcode}")
                return "error"
            self._record_send(client_id, "api_accepted", ret, errcode)
            return "ok"
        except Exception as e:
            self._record_send(client_id, "unknown")
            log(f"微信发送结果未知: {type(e).__name__}，重试可能重复")
            return "error"

    def _record_send(self, client_id, status, ret=None, errcode=None):
        with _state_lock:
            self.state["last_send"] = {"id": client_id, "status": status,
                                       "at": datetime.now().isoformat(),
                                       "ret": ret, "errcode": errcode}
            self.save_state()
        log(f"send id={client_id} status={status} ret={ret} errcode={errcode}")
    ...
    def _throttle_send(self):
        ...
        while True:
            ...
            time.sleep(max(wait, 0.01))  # 醒来重新竞争名额，避免并发超限
    ...
    def run(self):
        if not _acquire_lock():
            return
        log(f"🌉 微信桥 {version_info()} 启动 port={_APP_SERVER_PORT}")
...
def status():
    ...
    print(f"app_server: {_APP_SERVER_URL}")
    print(f"codex_home: {SETTINGS.get('codex_home', '继承 Codex 默认配置')}")
    if not ACCOUNT_FILE.exists():
    ...
    if STATE_FILE.exists():
        ...
        print(f"last_send: {json.dumps(st.get('last_send'), ensure_ascii=False)}")
    q = 0
    if RETRY_QUEUE_FILE.exists():
