...
$procs = Get-CimInstance Win32_Process | Where-Object { $_.Name -match 'python' -and $_.CommandLine -match '(wechat_bridge|codex_weixin_bridge)\.py.*run' }
if ($procs) {
...
if (Test-Path -LiteralPath $stateFile) {
    ...
    "last_send=$($state.last_send | ConvertTo-Json -Compress)"
}
if (Test-Path -LiteralPath $logFile) {
    ...
    if ($tail -match '-14') {
        ...
        Write-Output '>>> API accepted a reply; handset delivery is not confirmed'
    } elseif ($tail -match 'api_accepted') {
        Write-Output '>>> API accepted a reply; confirm receipt on the phone'
